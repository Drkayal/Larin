...
# استيراد Pyrogram بدلاً من Telethon
from pyrogram import Client, filters
from pyrogram.types import Message, Audio
from pyrogram.enums import ParseMode

import config
from ZeMusic.core.pyrogram_client import pyrogram_manager  # تغيير اسم الوحدة
...
🔍 {search_query[:50]}"""
            ...
            async with self.conn_manager.db_connection() as conn:
                ...
                # الحصول على file_id من Pyrogram
                file_id = message.audio.file_id
                
                cursor.execute('''
                    INSERT OR REPLACE INTO channel_index 
                    (message_id, file_id, search_hash, title_normalized, artist_normalized, 
                     keywords_vector, original_title, original_artist, duration, file_size)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    message.id, file_id, search_hash,
                    normalized_title, normalized_artist, keywords,
    ...
    async def hyper_download(self, query: str) -> Optional[Dict]:
        ...
        try:
            ...
            # إضافة طرق البحث بترتيب الأولية
...
async def execute_parallel_download(event, user_id: int, start_time: float, task_id: str):
    ...
    try:
        # استخراج الاستعلام
        match = re.search(r"بحث\s+(.+)", event.text)
        if not match:
...
async def send_cached_from_database(event, status_msg, db_result: Dict, bot_client):
    ...
    try:
            ...
            if 'message_id' in db_result and bot_client:
                try:
                    if hasattr(config, 'CACHE_CHANNEL_ID') and config.CACHE_CHANNEL_ID:
                        ...
                        channel_msg = await bot_client.get_messages(config.CACHE_CHANNEL_ID, message_ids=db_result['message_id'])
                        if channel_msg and channel_msg.audio and channel_msg.audio.thumbs:
                            # استخراج الصورة المصغرة من الملف
                            thumb_path = channel_msg.audio.thumbs[0].file_id
        ...
        sent_message = await event.reply_audio(
            audio=db_result['file_id'],
        ...
        try:
            if 'message_id' in cache_result and bot_client:
                try:
                    if hasattr(config, 'CACHE_CHANNEL_ID') and config.CACHE_CHANNEL_ID:
                        # الحصول على الرسالة من قناة التخزين للحصول على الصورة المصغرة
                        channel_msg = await bot_client.get_messages(config.CACHE_CHANNEL_ID, message_ids=cache_result['message_id'])
                        if channel_msg and channel_msg.audio and channel_msg.audio.thumbs:
                            # استخراج الصورة المصغرة من الملف
                            thumb_path = channel_msg.audio.thumbs[0].file_id
                            LOGGER(__name__).info(f"📸 تم العثور على الصورة المصغرة من قناة التخزين")
...
async def search_in_telegram_cache(query: str, bot_client) -> Optional[Dict]:
    ...
    try:
        ...
        async for message in bot_client.get_chat_history(cache_channel, limit=search_limit):
            if not (message.caption and message.audio):
                continue
            ...
            uploader = message.audio.performer if message.audio.performer else "Unknown"
        ...
        if best_matches:
            ...
            try:
                file_info = {
                    ...
                    'file_size': best_result['message'].audio.file_size if best_result['message'].audio.file_size else 0
                }
                
                await save_to_database_cache(
                    best_result['file_id'],
                    best_result['message'].audio.file_unique_id,
...
#{query_normalized.replace(' ', '_')[:30]} #هاش_{search_hash}"""
        ...
        try:
            ...
            async def upload_to_storage():
                try:
                    sent_message = await bot_client.send_audio(
            ...
            if sent_message and sent_message.audio:
                ...
                enhanced_info = {
                    ...
                    'uploader': uploader,
                    'duration': duration,
                    'file_size': sent_message.audio.file_size or file_size,
                    'source': source,
    ...
    for path in paths:
        if path and os.path.exists(path):
            try:
                ...
                LOGGER(__name__).warning(f"⚠️ فشل حذف {path}: {e}")
...
async def execute_parallel_download_enhanced(event, user_id: int, start_time: float, task_id: str):
    ...
    try:
        ...
        try:
            parallel_result = await parallel_search_with_monitoring(query, event._client)
            ...
            if parallel_result and parallel_result.get('success'):
                ...
                if search_source == 'database':
                    ...
                    success = await send_cached_from_database(event, status_msg, parallel_result, event._client)
                    if success:
                    ...
                    success = await send_cached_from_telegram(event, status_msg, parallel_result, event._client)
                    if success:
...
async def process_smart_youtube_download(event, status_msg, query: str, user_id: int, start_time: float, task_id: str):
    ...
    try:
        ...
        if result:
            ...
            await send_audio_file(event, status_msg, result['audio_path'], result, query, event._client)
...
# --- أوامر المطور مع Pyrogram ---
async def cache_stats_handler(client, message):
...
async def clear_cache_handler(client, message):
    ...
    try:
        ...
        await message.reply(f"""🧹 **تم مسح كاش التخزين!**
...
⚡ سيتم إعادة بناء الكاش تلقائياً مع الاستخدام""")
        ...
        await message.reply(f"❌ خطأ في مسح الكاش: {e}")
...
async def download_song_smart(message, query: str):
    ...
    try:
        ...
        cache_result, telegram_result = await parallel_cache_search(query, message._client)
...
async def parallel_cache_search(query: str, bot_client) -> Tuple[Optional[Dict], Optional[Dict]]:
    ...
    try:
        ...
        try:
            ...
            LOGGER(__name__).debug("✅ تم إنشاء مهمة التليجرام")
        except Exception as e:
...
async def save_to_cache(video_id: str, title: str, artist: str, duration: int, file_path: str, audio_message, thumb_path: str = None) -> bool:
    ...
    try:
        ...
        try:
            import config
            from ZeMusic.core.pyrogram_client import pyrogram_manager
...
LOGGER(__name__).info("🚀 تم تحميل نظام التحميل الذكي الخارق المتطور V2 مع Pyrogram")
...
async def sync_channel_to_database(bot_client, force_sync: bool = False) -> Dict:
    ...
    try:
        ...
        async for message in bot_client.get_chat_history(cache_channel, limit=1000):
            if not (message.caption and message.audio):
                continue
            ...
            try:
                ...
                import re
                hash_match = re.search(r'هاش البحث.*?`([a-f0-9]+)`', message.caption)
                ...
                if existing:
                    ...
                    cursor.execute("""
                        ...
                        WHERE message_id = ?
                    """, (
                        message.audio.file_id, title_normalized, uploader_normalized,
                        keywords_vector, title, uploader, duration,
                        message.audio.file_size or 0, search_hash, message.id
                    ))
                    ...
                    """, (
                        ...
                        title, uploader, duration, message.audio.file_size or 0
                    ))
                    sync_stats['added'] += 1
...
async def force_channel_sync_handler(client, message):
    ...
    try:
        await message.reply("🔄 **بدء مزامنة قناة التخزين...**")
        
        result = await sync_channel_to_database(client, force_sync=True)
        
        if 'error' in result:
            await message.reply(f"❌ **خطأ في المزامنة:** {result['error']}")
        elif 'skipped' in result:
            await message.reply(f"⏭️ **تم تخطي المزامنة:** {result['reason']}")
        else:
...
async def verify_cache_channel(bot_client) -> Dict:
    ...
    try:
        ...
        try:
            chat = await bot_client.get_chat(cache_channel)
            ...
            try:
                message_count = 0
                async for _ in bot_client.get_chat_history(cache_channel, limit=1):
                    message_count += 1
                
                channel_info['message_count'] = message_count
                LOGGER(__name__).info(f"📊 عدد الرسائل في القناة: {message_count}")
                
            except Exception as count_error:
                channel_info['message_count_error'] = str(count_error)
            ...
            return {
                'status': 'error',
                'message': f"فشل الوصول إلى القناة: {str(e)}",
                'solution': 'تأكد من أن البوت عضو في القناة وأن لديه الصلاحيات اللازمة'
            }
