...
_ALLOWED_PASSTHRU = {"تشغيل", "بحث", "song", "/song", "vplay", "cplay", "cvplay"}
...
async def subscription(_, __: Client, message: Message):
	...
	if not channel_slug:
		return True
	# allow certain commands to pass
	try:
		cmd0 = (message.command[0] if hasattr(message, "command") and message.command else "").lower()
	except Exception:
		cmd0 = ""
	if cmd0 in _ALLOWED_PASSTHRU:
		return True
	try:
